...
export const generateMetadata = () =>
  buildMetadata({
    title: "Videoer — Se Nordica i brug",
      ...
      <div className="container-page">
        ...
        {videos.length === 0 ? (
          <div className="mt-12 rounded-xl2 border border-dashed border-brand-goldline bg-brand-charcoal/50 p-12 text-center">
            <p className="text-brand-cream2/70">Der er endnu ingen videoer tilgængelige.</p>
            <p className="mt-1 text-sm text-brand-cream2/50">Vi uploader videoer her meget snart.</p>
          </div>
        ) : (
          <div className="mt-10 grid gap-6 sm:grid-cols-2">
            {videos.map((v) => (
              <figure key={v.id} className="overflow-hidden rounded-xl2 border border-brand-goldline bg-brand-ink">
