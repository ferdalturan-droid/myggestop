...
function keyFor(type: string) {
  if (type === "PERDE") return "perde_saved_records";
  if (type === "UNIFIED") return "unified_saved_records";
  return "imalat_saved_records";
}
...
export async function GET(req: NextRequest) {
  ...
  const type = req.nextUrl.searchParams.get("type") || "UNIFIED";
...
export async function POST(req: NextRequest) {
  ...
  const type = b.type === "PERDE" ? "PERDE" : b.type === "UNIFIED" ? "UNIFIED" : "SINEKLIK";
