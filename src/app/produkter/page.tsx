    ...
    <>
      <section className="bg-mesh section">
        <div className="container-page">
          ...
          />
          <img src="/produkter-banner.png" alt="Nordica produkter - plissédør og myggenet til vinduer og døre" className="mt-10 w-full rounded-xl2 border border-brand-goldline shadow-card" />
        ...
        <div className="container-page grid gap-10 lg:grid-cols-2">
          {products.map((p) => (
            <div key={p.id} className="reveal card p-8">
              <h3 className="text-2xl font-bold text-brand-cream">{p.name}</h3>
              <p className="mt-3 leading-relaxed text-brand-cream2/80">{p.description}</p>
              <dl className="mt-6 grid grid-cols-2 gap-4 text-sm">
                <div className="rounded-xl bg-brand-charcoal2 p-4">
                  <dt className="text-brand-cream2/60">Max bredde</dt>
                  <dd className="text-lg font-bold text-brand-cream">{p.maxWidthMm} mm</dd>
                </div>
                <div className="rounded-xl bg-brand-charcoal2 p-4">
                  <dt className="text-brand-cream2/60">Max højde</dt>
                  <dd className="text-lg font-bold text-brand-cream">{p.maxHeightMm} mm</dd>
                </div>
              </dl>
              {p.doubleDoorThresholdMm && (
                <p className="mt-4 rounded-xl border border-brand-blue/20 bg-brand-blue/5 p-4 text-sm text-brand-blue">
