...
export default function ImalatCalc() {
  ...
  function yazdir() {
    ...
    <style>body{font-family:Arial,sans-serif;color:#111;padding:28px;max-width:800px;margin:0 auto}h2{font-size:15px;margin:22px 0 8px;text-transform:uppercase;color:#3f9c12}.head{display:flex;justify-content:space-between;border-bottom:3px solid #11241c;padding-bottom:12px}.brand{font-weight:800;font-size:24px}.brand span{color:#5cc524}table{width:100%;border-collapse:collapse;font-size:13px;margin-top:4px}th,td{border:1px solid #ccc;padding:6px 8px;text-align:left}th{background:#f1f5f3;font-size:11px;color:#555}@media print{button{display:none}}</style></head><body>
    <div class="head"><div><div class="brand">MYGGE<span>STOP</span></div><div style="font-size:13px;color:#555">İŞ EMRİ - Kesim listesi</div></div>
    <div style="text-align:right;font-size:13px"><b>${musteri || "-"}</b><br>${tel || ""}<br>${adres || ""}<br>${new Date().toLocaleString("tr-TR")}</div></div>
    <h2>Pencereler</h2><table><thead><tr><th>#</th><th>Sistem</th><th>Tip</th><th>Model</th><th>Adet</th><th>EN</th><th>BOY</th></tr></thead><tbody>${wr}</tbody></table>
    <h2>Kesim listesi (toplam)</h2><table><thead><tr><th>Sistem</th><th>Parça</th><th>Ölçü (cm)</th><th>Adet</th></tr></thead><tbody>${kl}</tbody></table>
  ...
  return (
    <div>
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex items-center gap-2"><button onClick={yeni} className="btn-secondary py-2 text-sm">Yeni</button><button onClick={yazdir} className="btn-secondary py-2 text-sm">Yazdır / PDF</button><button onClick={kaydet} className="btn-primary py-2 text-sm">Kaydet</button></div>
      </div>
      ...
      <div className="overflow-hidden rounded-xl border border-brand-line">
        <div className="hidden bg-brand-mist px-3 py-2 text-xs font-semibold uppercase tracking-wide text-brand-ink2/60 sm:grid sm:grid-cols-[28px_80px_76px_96px_60px_72px_72px_70px_30px] sm:gap-2">
          <span>#</span><span>Sistem</span><span>Tip</span><span>Model</span><span>Adet</span><span>EN</span><span>BOY</span><span>Kesildi</span><span></span>
        </div>
        {rows.map((r, i) => {
          const ps = parts(r); const open = openUid === r.uid;
          return (
            <div key={r.uid} className={`border-t border-brand-line ${r.done ? "bg-green-50/60" : ""}`}>
              <div className="grid grid-cols-2 gap-2 px-3 py-2 sm:grid-cols-[28px_80px_76px_96px_60px_72px_72px_70px_30px] sm:items-center">
                <button onClick={() => setOpenUid(open ? null : r.uid)} className="flex items-center gap-1 text-sm font-bold text-brand-greendark" title="Detay">{i + 1}<span className="text-xs">{open ? "▲" : "▼"}</span></button>
              ...
              {open && ps && (
                <div className="border-t border-brand-line bg-brand-mist/40 px-4 py-3">
                  <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-brand-ink2/60">Pencere {i + 1} — {r.tip === "DUBLE" ? "Duble" : "Tek"} {r.sys} · {r.en}×{r.boy} cm</p>
                  <div className="grid gap-1.5 text-sm sm:grid-cols-2">
                    {ps.map((p) => (
                      <div key={p.label} className="flex justify-between rounded bg-white px-3 py-1.5">
                        <span className="text-brand-ink2/70">{p.label}</span>
                        <span className="font-semibold text-brand-ink">{p.kind === "pile" ? `${f(p.len!)} kat` : p.kind === "count" ? `${p.qty} adet` : `${p.qty} adet × ${f(p.len!)} cm`}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <table className="w-full text-sm"><thead><tr className="bg-brand-mist text-left text-xs uppercase tracking-wide text-brand-ink2/60"><th className="px-3 py-2">✓</th><th className="px-3 py-2">Sistem</th><th className="px-3 py-2">Parça</th><th className="px-3 py-2">Ölçü</th><th className="px-3 py-2">Adet</th></tr></thead>
              <tbody className="divide-y divide-brand-line">
                {liste.arr.map((p, idx) => { const k = `${p.sys}|${p.label}|${p.len.toFixed(2)}`; const d = doneKeys.includes(k); return (
                  <tr key={idx} className={d ? "bg-green-50/60" : ""}>
                    <td className="px-3 py-2"><input type="checkbox" checked={d} onChange={() => toggleKey(k)} /></td>
                    <td className="px-3 py-2 text-brand-ink2/70">{p.sys}</td>
                    <td className={`px-3 py-2 font-medium ${d ? "text-brand-ink2/40 line-through" : "text-brand-ink"}`}>{p.label}</td>
                    <td className={`px-3 py-2 font-semibold ${d ? "text-brand-ink2/40 line-through" : "text-brand-ink"}`}>{f(p.len)} cm</td>
                    <td className="px-3 py-2 text-brand-ink">{p.qty} adet</td>
                  </tr>); })}
