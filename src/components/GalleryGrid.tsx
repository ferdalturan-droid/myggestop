        ...
        {filters.map((f) => (
          <button
            ...
            className={`rounded-full px-4 py-2 text-sm font-semibold transition ${
              active === f.key ? "bg-brand-blue text-brand-black shadow-glow" : "bg-brand-charcoal text-brand-cream2 border border-brand-goldline hover:border-brand-blue"
            }`}
          >
            {f.label}
          </button>
      ...
      {loading ? (
        <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="aspect-[4/3] animate-pulse rounded-xl2 bg-brand-charcoal" />
          ))}
        </div>
      ) : items.length === 0 ? (
        <div className="mt-12 rounded-xl2 border border-dashed border-brand-goldline bg-brand-charcoal/50 p-12 text-center">
          <p className="text-brand-cream2/70">Der er endnu ingen billeder i denne kategori.</p>
          <p className="mt-1 text-sm text-brand-cream2/50">Galleriet udfyldes lobende — kig forbi igen snart.</p>
        </div>
      ) : (
        <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((it) => (
            <figure key={it.id} className="group relative overflow-hidden rounded-xl2 border border-brand-goldline bg-brand-ink">
              {it.type === "video" ? (
